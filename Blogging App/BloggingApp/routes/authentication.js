...
module.exports = (router) => {
            ...
            if (!req.body.password) {
                ...
                user.save((err) => {
                    ...
                    if (err) {
                        ...
                        if (err.code === 11000) {
                            res.json({ success: false, message: 'Name or Username or e-mail already exists' }); // Return error
                        } else {
                            ...
                            if (err.errors) {
                                ...
                                if (err.errors.email) {
                                    ...
                                    if (err.errors.username) res.json({success: false, message: err.errors.username.message}); else {
    ...
    // Route to check if user's name is available for registration
    
    router.get('/checkName/:name', (req, res) => {
        // Check if username was provided in paramaters
        if (!req.params.name) {
        res.json({ success: false, message: 'Name was not provided' }); // Return error
    } else {
        // Look for name in database
        User.findOne({ name: req.params.name }, (err, user) => { // Check if connection error was found
            if (err) {
                res.json({ success: false, message: err }); // Return connection error
            } else {
                // Set user's name was found
                res.json({ success: true, message: 'Name is available' }); // Return as vailable name
    }
    });
    }
});
   ...
   // LOGIN ROUTE
    ...
    router.get('/profile', (req, res) => {
        ...
        User.findOne({ _id: req.decoded.userId }).select('username email name').exec((err, user) => {
    ...
    router.get('/publicProfile/:username', (req, res) => {
        ...
        User.findOne({ username: req.params.username }).select('username email name').exec((err, user) => {
            // Check if error was found
            if (err) {
